  ...
  for (const play of input.plays) {
    ...
    if (
      ...
      items.push({
        ...
        evidence: `Skipping a step on ${play.playName} activities has hurt win rate the most — deals win ${dropPercent(penalty)}% less often when that happens.`,
      ...
      items.push({
        ...
        playId: play.playId,
        playName: play.playName,
        evidence: `${play.playName} activities skip a recommended prerequisite fairly often, but the win-rate difference is too small to change the play yet.`,
        sampleSize: play.closedOpportunityCount,
        confidence: play.win.confidence,
        href: `/?modal=play&playId=${play.playId}`,
      })
  ...
  for (const prereq of input.prerequisites) {
    ...
    if (
      ...
      items.push({
        ...
        evidence: `“${prereq.text}” is often missing on ${prereq.playName} activities, but win rate looks about the same either way — worth asking if we still need this prerequisite.`,
      ...
      items.push({
        ...
        playId: prereq.playId,
        playName: prereq.playName,
        evidence: `When “${prereq.text}” is missing on ${prereq.playName} activities, deals win ${dropPercent(penalty)}% less often.`,
        sampleSize: prereq.win.metN + prereq.win.unmetN,
